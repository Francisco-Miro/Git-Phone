...
class PhoneApiBackEndServer {
 ...
 async viewComment(req, res) {
   try {
     ...
     const stored = await collection.find({ model: model }).toArray();
      const usersCollection = db.collection("Users");
      const response = await Promise.all(stored.map(async comment => {
       const userDoc = await usersCollection.findOne({ user: comment.user });
       return {
         model: comment.model,
         user: comment.user,
         comment: comment.comment,
         profilePicture: userDoc ? userDoc.profilePicture : null
       };
     }));
      res.json(response);
   } catch (error) {
